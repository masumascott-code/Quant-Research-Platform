...
import { configService, type SignalRuntimeConfig } from "../core/config";
...
function calcRVOL(candles: CandleData[], lookback: number): number {
...
function detectHigherHighHigherLow(candles: CandleData[], lookbackPeriod: number): boolean {
...
function detectLowerHighLowerLow(candles: CandleData[], lookbackPeriod: number): boolean {
  if (candles.length < lookbackPeriod) return false;
  const recent = candles.slice(-lookbackPeriod);
  const highs = recent.map(c => c.high);
  const lows = recent.map(c => c.low);
...
function detectBreakout(candles: CandleData[], ema20: number, lookbackPeriod: number): { detected: boolean; level: number } {
  if (candles.length < lookbackPeriod) return { detected: false, level: 0 };
  const lookback = candles.slice(-lookbackPeriod, -1);
...
function detectBreakdown(candles: CandleData[], ema20: number, lookbackPeriod: number): { detected: boolean; level: number } {
  if (candles.length < lookbackPeriod) return { detected: false, level: 0 };
  const lookback = candles.slice(-lookbackPeriod, -1);
...
function detectFakeBreakout(candles: CandleData[], level: number, dir: "LONG" | "SHORT", config: SignalRuntimeConfig): boolean {
  const last = candles[candles.length - 1];
  ...
  if (dir === "LONG") {
    ...
    const volumeWeak = last.volume < prev.volume * config.fakeoutVolumeRatio;
    const noClose = last.close < level;
    return wickRatio > config.fakeoutWickRatio || volumeWeak || noClose;
  } else {
...
function detectRetest(candles: CandleData[], level: number, tolerance: number): boolean {
...
function detectRetestShort(candles: CandleData[], level: number, tolerance: number): boolean {
  if (candles.length < 3) return false;
  const prev = candles[candles.length - 2];
  const last = candles[candles.length - 1];
...
function isBullishCandle(c: CandleData, bodyRatio: number): boolean {
  const body = c.close - c.open;
  const range = c.high - c.low;
  return c.close > c.open && range > 0 && (body / range) > bodyRatio;
}

function isBearishCandle(c: CandleData, bodyRatio: number): boolean {
...
export function scoreToConfidence(score: number): Confidence {
  const config = configService.getSync().signal;
  if (score >= config.confidenceExtremeScore) return "Extreme";
  if (score >= config.confidenceVeryHighScore) return "Very High";
  if (score >= config.confidenceHighScore) return "High";
  if (score >= config.confidenceMediumScore) return "Medium";
  return "Low";
}
...
function checkTimeframeAlignment(
  ...
  const m15Ema20 = calcEMA(m15Closes, config.emaFastPeriod);
  const m15Ema50 = calcEMA(m15Closes, config.emaSlowPeriod);
  const h1Ema20 = calcEMA(h1Closes, config.emaFastPeriod);
  const h1Ema50 = calcEMA(h1Closes, config.emaSlowPeriod);
  const m5Ema20 = calcEMA(m5Closes, config.emaFastPeriod);
  ...
  return {
    aligned: alignedCount >= config.minTimeframeAlignment,
    summary: checks.join(" | "),
    score: alignedCount,
  };
}
...
function detectSetupType(
  ...
  if (rvol >= config.volumeExpansionRvol) return "Volume Expansion";

  if (hasBreakout && hasRetest) return "Breakout Retest";
  if (hasBreakout) return "Resistance Breakout";

  const nearEma20 = Math.abs(price - ema20) / ema20 < config.emaPullbackTolerance;
  if (nearEma20 && dir === "LONG" && price > ema50) return "EMA Pullback";
  if (nearEma20 && dir === "SHORT" && price < ema50) return "EMA Pullback";

  const recentLows = candles.slice(-config.supportResistanceLookback).map(c => c.low);
  const supportLevel = Math.min(...recentLows.slice(0, config.supportResistanceBaseLookback));
  const nearSupport = Math.abs(price - supportLevel) / supportLevel < config.supportResistanceTolerance;
  if (nearSupport && dir === "LONG") return "Support Bounce";

  const recentHighs = candles.slice(-config.supportResistanceLookback).map(c => c.high);
  const resistanceLevel = Math.max(...recentHighs.slice(0, config.supportResistanceBaseLookback));
  const nearResistance = Math.abs(price - resistanceLevel) / resistanceLevel < config.supportResistanceTolerance;
...
export function analyzeForLong(
  ...
  if (candles15m.length < config.minCandles15m) return null;
  ...
  let breakoutScore = 0;
  const { detected: hasBreakout, level: resistanceLevel } = detectBreakout(candles15m, ema20, config.breakoutLookback);
  if (hasBreakout) {
    breakoutScore += 10;
    if (!detectFakeBreakout(candles15m, resistanceLevel, "LONG", config)) {
      breakoutScore += 5;
    }
  }
  ...
  // ── Retest Score (10 pts) ─────────────────────────────────────────────────
  let retestScore = 0;
  const hasRetest = detectRetest(candles15m, ema20, config.retestTolerance) ||
    detectRetest(candles15m, resistanceLevel, config.setupRetestTolerance);
  if (hasRetest) retestScore += 7;
  if (isBullishCandle(candles15m[candles15m.length - 1], config.candleBodyRatio)) retestScore += 3;
  ...
  let momentumScore = 0;
  if (isBullishMomentum(candles15m, config.momentumCandles)) momentumScore += 3;
  ...
  if (mtf && mtf.m5.length >= config.mtfMinCandles && mtf.h1.length >= config.mtfMinCandles) {
    const tfResult = checkTimeframeAlignment(candles15m, mtf.h1, mtf.m5, "LONG", config);
    tfSummary = tfResult.summary;
    if (!tfResult.aligned) return null;
    tfBonus = tfResult.score * config.timeframeScoreMultiplier;
  }

  let score = trendScore + emaScore + volumeScore + rvolScore + breakoutScore + retestScore + structureScore + momentumScore;
  score = Math.min(config.maxScore, score + tfBonus);

  if (score < config.minAnalysisScore) return null;

  // ── SL/TP calculation ─────────────────────────────────────────────────────
  const swingLow = Math.min(...candles15m.slice(-config.swingLookback).map(c => c.low));
  const slCandidates = [
    swingLow - atr14 * config.atrSwingBuffer,
  ...
  if (risk <= 0 || risk / currentPrice > config.maxRiskPercent) return null;

  const tp1 = currentPrice + risk * config.tp1RiskMultiple;
  const tp2 = currentPrice + risk * config.tp2RiskMultiple;
  const tp3 = currentPrice + risk * config.tp3RiskMultiple;
  const rrRatio = (tp2 - currentPrice) / risk;

  if (rrRatio < config.minRrRatio) return null;

  const setupType = detectSetupType(candles15m, ema20, ema50, hasBreakout, hasRetest, rvol, "LONG", config);
  const confidence = scoreToConfidence(score);
  const grade = score >= config.gradeAPlusScore ? "A+" : "A";
  ...
  const whyLong = `${detectHigherHighHigherLow(candles15m, config.structureLookback) ? "Higher Highs + Higher Lows structure. " : ""}EMA20 > EMA50 confirms uptrend. ${hasBreakout ? "Clean breakout above resistance." : "Price holds above EMA support."}`;
...
export function analyzeForShort(
  ...
  // ── Breakout Score (15 pts) ───────────────────────────────────────────────
  let breakoutScore = 0;
  const { detected: hasBreakdown, level: supportLevel } = detectBreakdown(candles15m, ema20, config.breakoutLookback);
  if (hasBreakdown) {
    breakoutScore += 10;
    if (!detectFakeBreakout(candles15m, supportLevel, "SHORT", config)) {
      breakoutScore += 5;
    }
  }
  ...
  // ── Retest Score (10 pts) ─────────────────────────────────────────────────
  let retestScore = 0;
  const hasRetest = detectRetestShort(candles15m, ema20, config.retestTolerance) ||
    detectRetestShort(candles15m, supportLevel, config.setupRetestTolerance);
  if (hasRetest) retestScore += 7;
  if (isBearishCandle(candles15m[candles15m.length - 1], config.candleBodyRatio)) retestScore += 3;
  ...
  // ── Momentum Score (5 pts) ────────────────────────────────────────────────
  let momentumScore = 0;
  if (isBearishMomentum(candles15m, config.momentumCandles)) momentumScore += 3;
  const last = candles15m[candles15m.length - 1];
  const lastBody = Math.abs(last.close - last.open);
  const lastRange = last.high - last.low;
  if (lastRange > 0 && lastBody / lastRange > 0.6) momentumScore += 2;
  ...
  let tfSummary = "15m only";
  if (mtf && mtf.m5.length >= config.mtfMinCandles && mtf.h1.length >= config.mtfMinCandles) {
    const tfResult = checkTimeframeAlignment(candles15m, mtf.h1, mtf.m5, "SHORT", config);
    tfSummary = tfResult.summary;
    if (!tfResult.aligned) return null;
    tfBonus = tfResult.score * config.timeframeScoreMultiplier;
  }
  ...
  // ── SL/TP calculation ─────────────────────────────────────────────────────
  const swingHigh = Math.max(...candles15m.slice(-config.swingLookback).map(c => c.high));
  const slCandidates = [
    swingHigh + atr14 * config.atrSwingBuffer,
    ema20 + atr14 * config.atrEmaBuffer,
    currentPrice + atr14 * config.atrMaxStopBuffer,
  ...
  const setupType = detectSetupType(candles15m, ema20, ema50, hasBreakdown, hasRetest, rvol, "SHORT", config);
  const confidence = scoreToConfidence(score);
  const grade = score >= config.gradeAPlusScore ? "A+" : "A";
  ...
  const whyLong = `${detectLowerHighLowerLow(candles15m, config.structureLookback) ? "Lower Highs + Lower Lows structure. " : ""}EMA20 < EMA50 confirms downtrend. ${hasBreakdown ? "Clean breakdown below support." : "Price rejected below EMA resistance."}`;
