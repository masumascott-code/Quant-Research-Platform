...
import { nonNegativeIntegerEnv, positiveIntegerEnv, validateProductionEnvironment } from "./env";

test("positiveIntegerEnv rejects invalid scheduler intervals", () => {
  withEnv("SCHEDULE_DAILY_REPORT_MS", "0", () => {
    assert.throws(
      () => positiveIntegerEnv("SCHEDULE_DAILY_REPORT_MS", 86_400_000, "positive integer milliseconds"),
...
test("nonNegativeIntegerEnv rejects invalid queue numeric env values", () => {
  withEnv("QUEUE_JOB_BACKOFF_MS", "-1", () => {
    assert.throws(
      () => nonNegativeIntegerEnv("QUEUE_JOB_BACKOFF_MS", 5_000, "non-negative integer milliseconds"),
...
test("integer env helpers keep valid numeric env values", () => {
  withEnv("WORKER_CONCURRENCY", "4", () => {
    assert.equal(positiveIntegerEnv("WORKER_CONCURRENCY", 2, "positive integer worker concurrency"), 4);
  });

  withEnv("QUEUE_REMOVE_COMPLETE_COUNT", "0", () => {
    assert.equal(nonNegativeIntegerEnv("QUEUE_REMOVE_COMPLETE_COUNT", 1_000, "non-negative integer count"), 0);
  });
});
